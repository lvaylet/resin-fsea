...
Commits and pushes directly to the `master` branch most of the time. I created two `bulma` and `unit-mocha` branches to demonstrate my understanding of branches. The TODO list is handled at the bottom of this `README.md`. | Use branches, peer programming reviews and pull requests to a protected `master` branch to improve quality and security. Tickets (like TODO items, bugs and improvements) could be handled in GitHub issues or JIRA (with smart commits enabled to track progress automatically).

# TODO

- Add unit tests for `Subscriber` component and include `npm run test:unit` in multi-stage build for production (drawing inspiration from https://codefresh.io/docker-tutorial/node_docker_multistage/).
